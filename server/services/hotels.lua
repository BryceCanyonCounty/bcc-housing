...
BccUtils.RPC:Register('bcc-housing:HotelBought', function(params, cb, src)
    ...
    local ownedHotels = (result and result[1] and result[1].hotels) or 'none'

    local ownedHotelsTable = (ownedHotels == 'none') and {} or (json.decode(ownedHotels) or {})
...
BccUtils.RPC:Register('bcc-housing:RegisterHotelInventory', function(params, cb, src)
    ...
    if not hotelId then if cb then cb(false) end return end

    local invId = 'bcc-housinginv:' .. tostring(hotelId) .. tostring(charId)
