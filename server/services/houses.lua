  ...
  exports.oxmysql:execute("SELECT * FROM bcchousing", function(result)
    if #result > 0 then
      for k, v in pairs(result) do
        ...
        --Wait(50)
        VorpInv.registerInventory('Player_' .. v.houseid .. '_bcc-houseinv', _U("houseInv"), tonumber(v.invlimit), true,
          true, true)
...
RegisterServerEvent('bcc-housing:GetOwnerFurniture',
  function(houseid) --event to handler returning all owned furniture to open the selling menu
    local param = { ['houseid'] = houseid }
    local _source = source
    local result = MySQL.query.await("SELECT * FROM bcchousing WHERE houseid=@houseid", param)
    if result[1] then
...
RegisterServerEvent('bcc-housing:FurnSoldRemoveFromTable',
  function(furnTable, houseid, wholeFurnTable, wholeFurnTableKey) --selling furn handler
    local _source = source
    local character = VORPcore.getUser(_source).getUsedCharacter
    ...
    if result and #result > 0 then
      ...
      if insertionAmount > 0 then
        if character.money >= insertionAmount then
          ...
          MySQL.update("UPDATE bcchousing SET ledger = ledger + ? WHERE houseid = ?", { insertionAmount, houseIdNumber },
            function(affectedRows)
              if affectedRows > 0 then
                VORPcore.NotifyLeft(_source, _U("ledgerAmountInesrted") .. " $" .. insertionAmount, "", "inventory_items",
                  "money_moneystack", 5000)
              else
        ...
        VORPcore.NotifyLeft(_source, "Maximum amount of money is already stored", "", "menu_textures", "menu_icon_alert",
          5000)
      end
    else
...
RegisterServerEvent('bcc-housing:CheckLedger', function(houseid) --check ledger handler
  ...
  local result = MySQL.query.await("SELECT * FROM bcchousing WHERE houseid=@houseid", param)
  if #result > 0 then
    VORPcore.NotifyLeft(_source, tostring(result[1].ledger) .. '/' .. tostring(result[1].tax_amount), "", "menu_textures",
